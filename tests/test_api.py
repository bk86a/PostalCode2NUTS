...
class TestFaroeIslandsAPI:
    def test_lookup_fo(self, client):
        r = client.get("/lookup", params={"country": "FO", "postal_code": "100"})
    ...
    def test_pattern_fo(self, client):
        ...
        body = r.json()
        assert body["country_code"] == "FO"
        assert "100" in body["example"]

    def test_pattern_list_includes_fo(self, client):
        r = client.get("/pattern")
        assert r.status_code == 200
        assert "FO" in r.json()
...
class TestHealthEndpoint:
    ...
    def test_includes_patterns_version(self, client):
        ...
        assert data["patterns_version"] == "1.4"
