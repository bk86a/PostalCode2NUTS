...
| `PC2NUTS_TRUSTED_TOKENS` | `""` (empty — bypass disabled) | Comma-separated list of opaque tokens that bypass the per-IP rate limit when sent via `Authorization: Bearer <token>`. **Each token must be at least 32 characters** — the service refuses to start otherwise, because an invalid token is rejected before the rate limiter runs and a short one could be guessed unmetered. Continues to work as a union with the DB-backed registry below; set this only as a disaster-recovery fallback or for env-var-only deployments. See [Authentication & rate-limit bypass](#authentication--rate-limit-bypass) for the operator runbook. |
...
| `PC2NUTS_CORS_ORIGINS` | `*` | Comma-separated list of allowed CORS origins. The wildcard default suits the public read-only API; **set a specific origin (e.g. `https://example.com`) for any internal or LAN deployment**, where `*` would let any site the operator visits query the service through their browser. Empty string disables CORS middleware. Credentials are never allowed cross-origin. |
| `PC2NUTS_FORWARDED_ALLOW_IPS` | `127.0.0.1` | Which peers may set `X-Forwarded-For` / `X-Forwarded-Proto`. Set it to the address or CIDR of the reverse proxy in front of the service (e.g. `172.17.0.1`, `10.0.0.0/8`). **Never `*`** — see [Reverse proxies](#deployment-notes). Read by the container `CMD`, not by the app. |
| `PC2NUTS_MAX_DOWNLOAD_MB` | `512` | Ceiling on any single remote body the worker buffers (TERCET/NSPL zips, NUTS polygons, names CSV). A download that passes it is abandoned rather than exhausting memory. |
| `PC2NUTS_MAX_ESTIMATES_DOWNLOAD_MB` | `64` | Same ceiling for `PC2NUTS_ESTIMATES_REFRESH_URL` bodies. The bundled CSV is ~0.2 MB. |
...
- **Reverse proxies:** The image runs uvicorn with `--proxy-headers --forwarded-allow-ips "${PC2NUTS_FORWARDED_ALLOW_IPS:-127.0.0.1}"`, so `X-Forwarded-Proto`, `X-Forwarded-For`, and `X-Forwarded-Host` are honoured **only from the peers you list**. Set `PC2NUTS_FORWARDED_ALLOW_IPS` to the proxy's address or CIDR; leave it unset only when the proxy talks to the container over loopback. Do not set it to `*`: uvicorn then trusts the header from any peer and takes its *leftmost* entry — the value the client sent — so any caller could mint a fresh rate-limit bucket per request and forge the client IP in the access log, even behind a proxy that appends the header correctly. If the header is not trusted, `/` link URLs and rate-limit keying fall back to the proxy's own address, which buckets all traffic together — that is the symptom of a missing `PC2NUTS_FORWARDED_ALLOW_IPS`.
- **Rate limiting:** Limits are per-client IP, taken from `X-Forwarded-For` only when the peer is trusted per the previous point. The proxy must also *set* the header — appending is enough once `PC2NUTS_FORWARDED_ALLOW_IPS` names it.
...
| uvicorn flags | `--proxy-headers --forwarded-allow-ips "${PC2NUTS_FORWARDED_ALLOW_IPS:-127.0.0.1}" --no-access-log` (proxy headers honoured only from the listed peers — set `PC2NUTS_FORWARDED_ALLOW_IPS` to your proxy; access log disabled so `/resolve` street/city query params never reach stdout — use `PC2NUTS_ACCESS_LOG_FILE` for sanitized access logging) |
